...
#ifdef APRILFOOL
  /* Quick april fool's day check */
  if ((tm->tm_mon + 1) == 4 && tm->tm_mday == 1)
    aprfool = 1;
  else
  aprfool = 0;
#endif
...
int chr;
{
  /* any char between 2 and 10 will be changed */
  if (!randchar)
    randchar = 2 + (int) (10.0 * rand() / (RAND_MAX + 1.0));

  if (++randcnt >= randchar && !ispunct (chr) && !isspace (chr))
  {
    randcnt = 0;
    randchar = 0;
    do {
      randchar = 33 + (int) (91.0 * rand() / (RAND_MAX + 1.0));
    } while (islower (chr) ? !islower (randchar) : !isupper (randchar));
   my_putchar (randchar);
   randchar = 0;
  }
  else
