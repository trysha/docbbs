...
alarmclock()
{
  ...
  if (logintime == 6000)
  {
    colorize("\n\n\a@CYou been on hundred hour.  That long enough to fight respectable war.  Goodbye.  *gong*\n\n");
    my_putchar(BEL);
    my_exit(10);
  }
...
int noflush = 1;
  ...
  while (i > DEL)
  {
    ...
    /* Wobbly prompted this one.  Sometimes you get those users
     * that you can't twit.  So phuck 'em.  I don't know what's
     * worse -- dropping characters or just buffering them.
     */

    if (ouruser && ouruser->f_phucked)
      flush_input (1);
